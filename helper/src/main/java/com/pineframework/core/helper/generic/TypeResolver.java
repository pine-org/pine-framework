...
package com.pineframework.core.helper.generic;
...
public final class TypeResolver {
    ...
    static {
        ...
        try {
            ...
            for (Method method : Object.class.getDeclaredMethods()) OBJECT_METHODS.put(method.getName(), method);
     ...
     *                                       the following types: {@link Class}, {@link TypeVariable},
     *                                       {@link WildcardType}, {@link ParameterizedType},
     *                                       {@link GenericArrayType}.
     * @throws UnsupportedOperationException if {@code type} (or a type that it references) is a
     *                                       {@link WildcardType} that
     *                                       does not have exactly one upper bound, or does not have no lower bounds.
     * @throws UnsupportedOperationException if {@code type} (or a type that it references) is a
     *                                       {@link GenericArrayType}
     *                                       whose generic component type cannot be reified to an instance of
     *                                       {@link Class}.
     */
     ...
     */
    private static Type reify(Type genericType,
                              final Map<TypeVariable<?>, Type> typeVariableMap,
                              Map<Type, Type> cache) {
        // Terminal case.
        if (genericType instanceof Class<?>) return genericType;
        ...
        if (genericType instanceof ParameterizedType) {
            ...
            for (int i = 0; i < genericTypeArguments.length; i++) {
                ...
                if (genericTypeArguments[i] == null) return parameterizedType;
            ...
            if (genericComponentType == reifiedComponentType) return genericComponentType;
     ...
     * yield the parameterized type {@code Optional<List<Number>>}. Note that not the raw type
     ...
     * @throws UnsupportedOperationException if {@code type} (or a type that it references)
     *                                       is a {@link WildcardType} that
     *                                       does not have exactly one upper bound,
     *                                       or does not have no lower bounds.
     * @throws UnsupportedOperationException if {@code type} (or a type that it references)
     *                                       is a {@link GenericArrayType}
     *                                       whose generic component type cannot be reified
     *                                       to an instance of {@link Class}.
     */
    ...
    public static Class<?>[] resolveRawArguments(Type genericType, Class<?> subType) {
        ...
        if (RESOLVES_LAMBDAS && subType.isSynthetic()) {
            ...
            if (fi != null && fi.isInterface()) functionalInterface = fi;
        }
     ...
     * @param subType to extract type variable information from
     * @return array of raw classes representing arguments for the {@code type} else {@code null}
     * if no type arguments are
     * declared
     */
    ...
    /**
     * Returns the generic {@code type} using type variable information from the {@code subType} else {@code null}
     * if the
    ...
    public static Type resolveGenericType(Class<?> type, Type subType) {
        Class<?> rawType;
        if (subType instanceof ParameterizedType) rawType = (Class<?>) ((ParameterizedType) subType).getRawType();
        else rawType = (Class<?>) subType;

        if (type.equals(rawType)) return subType;

        Type result;
        if (type.isInterface()) {
            for (Type superInterface : rawType.getGenericInterfaces()) {
                if (superInterface != null && !superInterface.equals(Object.class)) {
                    if ((result = resolveGenericType(type, superInterface)) != null) return result;
                }
            }
        ...
        if (superClass != null && !superClass.equals(Object.class)) {
            if ((result = resolveGenericType(type, superClass)) != null) return result;
        }
    ...
    private static void populateTypeArgs(ParameterizedType type, Map<TypeVariable<?>, Type> map, boolean depthFirst) {
        if (type.getRawType() instanceof Class) {
            ...
            if (type.getOwnerType() != null) {
                ...
                if (owner instanceof ParameterizedType) populateTypeArgs((ParameterizedType) owner, map, depthFirst);
            }
            ...
            for (Method m : functionalInterface.getMethods()) {
                if (!isDefaultMethod(m) && !Modifier.isStatic(m.getModifiers()) && !m.isBridge()) {
                    ...
                    if (returnTypeVar instanceof TypeVariable) {
                        ...
                        if (!returnType.equals(Void.class)) map.put((TypeVariable<?>) returnTypeVar, returnType);
                    }
    ...
    private static Member getMemberRef(Class<?> type) {
        ...
        for (int i = getConstantPoolSize(constantPool) - 1; i >= 0; i--) {
            ...
            if (member == null
                    ...
                    || member.getDeclaringClass().isAssignableFrom(type)) continue;
